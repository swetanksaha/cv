...
\texttt{tc}\newline
\texttt{Kernel}\newline
}{Multipath TCP (MPTCP) for Dual-band WiGig (802.11ad)/WiFi (802.11ac) networks}{NSF Funded}{}{}{
\begin{itemize}
...
\begin{itemize}
  \item Developed set of tools to enable utlra large-scale performance and power measurement of mobile devices involving data collection \& sync. across heterogeneous systems: Linux (Wireless AP), Android and Power monitor.
  ...
  \item Pubished 1 top-tier conference.
\end{itemize}
}
...
\begin{itemize}
  \item Set up the first ever software-defined based 60 GHz testbed that offer configurability at PHY/MAC/Network layers.
  \item Added several components (e.g., AGC) to existing NI codebase to enable realistic measurements.
  \item Developed set of tools to extensively automate the measurement cycle reducing time from hours to several minutes.
  \item Exposed user controls to enable measurements by external research groups (used by Rice University \& UT Austin).
\end{itemize}
}
